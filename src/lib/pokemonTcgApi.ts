...
const BASE_URL =
  (import.meta.env.VITE_POKEMON_TCG_API_BASE_URL as string | undefined)?.trim() ||
...
async function request<T>(path: string, opts: RequestOptions = {}): Promise<T> {
  ...
  try {
    ...
    throw new TcgApiError(
      err instanceof Error ? `Error de red: ${err.message}` : 'Error de red desconocido.',
      0
    );
  } finally {
    timedSignal.cleanup();
  ...
  if (!res.ok) {
    ...
    throw new TcgApiError(detail || `Error ${res.status} del servidor.`, res.status);
  }
...
export function parseSearchQuery(query: string): ParsedQuery {
  ...
  for (const word of words) {
    ...
    if (
      /^(sv|swsh|sm|xy|bw|col|hgss|pl|dp|ex|np|pop|me|zsv|rsv|base|cel|det|dv|ecard|fut|gym|mcd|neo|ru|si|tk)\d+[a-z0-9]*$/i.test(
        cleanWord
      )
    ) {
...
export function cleanLuceneQueryForLocalSearch(query: string): string {
  ...
  let clean = query.replace(
    /\b(id|set\.id|types|hp|supertype|subtypes|rarity|rules|set\.name)\s*:\s*(?:"[^"]*"|\[[^\]]*\]|[^\s)]+)/gi,
    ''
  );
...
export async function searchCards(
  ...
  if (
    params.name &&
    (/\b(name|id|types|hp|supertype|subtypes|rarity|rules|set\.id|set\.name)\s*:/i.test(
      params.name
    ) ||
...
export async function getCardById(id: string, opts: RequestOptions = {}): Promise<PokemonCard> {
...
export async function getSetById(id: string, opts: RequestOptions = {}): Promise<CardSet> {
...
export async function getCardsBySet(
  ...
  return searchCards({ setId, page, pageSize, orderBy: 'number' }, opts);
}
