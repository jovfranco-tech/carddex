...
export default function AISynergyFeed({ ownedCardNames }: AISynergyFeedProps) {
  ...
  const getTagColor = (tag: string) => {
    ...
    if (t.includes('consistencia') || t.includes('vel')) return '#2D9CDB'; // Blue
    if (t.includes('ataque') || t.includes('daño')) return '#EB5757'; // Red
    if (t.includes('control')) return '#F2C94C'; // Yellow
    if (t.includes('energía')) return '#9B51E0'; // Purple
    return 'var(--muted)';
  };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {Array.from({ length: 2 }).map((_, idx) => (
            <Surface key={idx} style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ height: 16, width: '40%', background: 'rgba(255,255,255,0.06)', borderRadius: 4, animation: 'shimmer 1.5s infinite linear' }} />
              <div style={{ height: 12, width: '90%', background: 'rgba(255,255,255,0.04)', borderRadius: 4, animation: 'shimmer 1.5s infinite linear' }} />
              <div style={{ height: 12, width: '70%', background: 'rgba(255,255,255,0.04)', borderRadius: 4, animation: 'shimmer 1.5s infinite linear' }} />
            </Surface>
          ))}
        </div>
      ) : error ? (
